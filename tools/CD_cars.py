...
def get_scars_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0, data_root=car_root):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = CarsDataset(data_dir=data_root, transform=train_transform, train=True)
    ...
    whole_test_dataset = CarsDataset(data_dir=data_root, transform=test_transform, train=False)
