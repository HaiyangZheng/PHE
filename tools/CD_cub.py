...
def get_cub_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0, download=False, data_root=cub_root):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = CustomCub2011(root=data_root, transform=train_transform, train=True, download=download)
    ...
    whole_test_dataset = CustomCub2011(root=data_root, transform=test_transform, train=False)
    test_dataset = subsample_classes(deepcopy(whole_test_dataset), include_classes=train_classes)
    # subsample_indices = subsample_instances(test_dataset, prop_indices_to_subsample=prop_train_labels)
